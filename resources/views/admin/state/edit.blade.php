...
@section('content')
    <header class="page-title-bar">
        ...
        <h1 class="page-title text-truncate">
            ...
            {{ __('Регион ID:') . " " . $state->id }}
        </h1>
    </header>
    <div class="page-section">
        <div class="section-block">
            <div class="card card-body">
                <form method="post" action="{{ route('admin.states.update', $state) }}" onsubmit="disableSubmitButton();">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-12 col-lg-6 form-group">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" value="{{ $state->name }}" name="name" required autofocus>
                            @error('name')
                        ...
                        <div class="col-12 col-lg-6">
                            <div class="row">
                                <div class="col-lg-6 form-group">
                                    ...
                                    <input type="text" class="form-control @error('iso_code') is-invalid @enderror" id="iso_code" value="{{ $state->iso_code }}" name="iso_code" maxlength="10" required>
                                    @error('iso_code')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                                ...
                                <div class="col-lg-6 form-group" lang="{{ app()->getLocale() }}">
                                    ...
                                    <select name="country_id" id="country_id"
                                        ...
                                        @forelse ($countries as $id => $name)
                                            <option value="{{ $id }}" {{ $state->country_id == $id ? 'selected' : '' }}>
                                                {{ $name }}
                                            </option>
                                        @empty
                                        @endforelse
